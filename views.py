...
def home(request):
    if request.method == 'POST':
        ...
        try:
            ...
            prediction = 'ML Server Offline — please start Flask server'
...
def map_symptoms_to_disease(post):
    ...
    # ── SYMPTOM CLUSTERS (more realistic) ──

    # Cholera → profuse watery diarrhea + vomiting + spread
    if 'watery_diarrhea' in symptoms:
        scores['Cholera'] += 4
    if 'vomiting' in symptoms and 'watery_diarrhea' in symptoms:
        scores['Cholera'] += 2

    # Dysentery → bloody stool + cramps
    if 'bloody_diarrhea' in symptoms:
        scores['Dysentery'] += 5
    if 'stomach_cramps' in symptoms:
        scores['Dysentery'] += 2

    # Gastroenteritis → short duration + vomiting + diarrhea
    ...
    # Typhoid → sustained fever pattern (not just fever)
    ...
    # Giardia → bloating + long duration
    ...
    # No treatment → higher infection probability
    if treatment == 'never':
        for k in scores:
            scores[k] *= 1.2

    # Poor sanitation → bacterial diseases
    if sanitation == 'poor':
        scores['Dysentery'] += 2
        scores['Typhoid'] += 1
    ...
    # ── INFO (unchanged but refined tone) ──
    disease_info = {
        'Cholera': {
            'description': 'Acute diarrheal infection causing severe dehydration due to contaminated water.',
            'go_to_hospital': 'Immediately if dehydration signs appear.',
            'immediate': 'Start ORS immediately and seek urgent care.'
        },
        'Typhoid': {
            'description': 'Bacterial infection with sustained high fever and systemic symptoms.',
            'go_to_hospital': 'If fever persists beyond 3 days.',
            'immediate': 'Rest, hydration, medical evaluation required.'
        },
        'Hepatitis A': {
            'description': 'Viral liver infection indicated by jaundice and fatigue.',
            'go_to_hospital': 'Yes, for liver monitoring.',
            'immediate': 'Rest, avoid fatty food, stay hydrated.'
        },
        'Dysentery': {
            'description': 'Intestinal infection with blood in stool and inflammation.',
            'go_to_hospital': 'If bleeding persists or fever present.',
            'immediate': 'ORS and medical consultation.'
        },
        'Gastroenteritis': {
            'description': 'Short-term infection causing vomiting and diarrhea.',
            'go_to_hospital': 'If unable to retain fluids.',
            'immediate': 'Rest and hydration.'
        },
        'Giardia': {
            'description': 'Parasitic infection causing long-term digestive issues and bloating.',
            'go_to_hospital': 'If symptoms persist beyond a week.',
            'immediate': 'Hydration and medical treatment needed.'
        },
    }
    ...
    return {
        ...
        'severity': severity,
        'probabilities': {k: round(v * 100, 1) for k, v in probabilities.items()},  # NEW (for charts 🔥)
...
def dashboard(request):
    # Always fetch past surveys first — needed for both GET and POST
    ...
    if request.method == 'POST':
        ...
        # Refresh past surveys after saving new one
        past_surveys = SurveyResult.objects.filter(
            user=request.user.username
        ).order_by('-created_at')[:5]
    ...
    # ✅ GET request — show empty form, no score/answers needed
    return render(request, 'dashboard.html', {
        'submitted':    False,
        'past_surveys': past_surveys,
    })
    ...
    if request.method == 'POST':
        ...
        if message:
            ...
            try:
                ...
                bot_response = "Sorry, the chat server is offline. Please try again later."
            request.session['chat_history'].append({
...
def score_quick_check(post):
    ...
    appearance_map = {
        'clear': 0, 'slight_cloudy': 1,
        'very_cloudy': 2, 'brown': 3, 'greenish': 3
    }
    scores.append(appearance_map.get(post.get('appearance', 'clear'), 0))

    smell_map = {
        'none': 0, 'earthy': 1, 'chlorine': 1,
        'rotten_egg': 2, 'chemical': 3, 'sewage': 3
    }
    ...
    event_map = {
        'none': 0, 'power_cut': 1, 'construction': 1,
        'illness': 2, 'flooding': 3
    }
    scores.append(max([event_map.get(e, 0) for e in events] or [0]))

    symptoms = post.getlist('symptoms')
    symptom_map = {
        'none': 0, 'stomach': 1, 'nausea': 1,
        'diarrhea': 2, 'vomiting': 2, 'multiple_sick': 3
    }
...
def get_quick_cause(post):
    ...
    if smell == 'rotten_egg':
        causes.append("rotten egg smell suggests hydrogen sulphide — likely anaerobic bacteria or corroded pipes")
    if smell == 'sewage':
        causes.append("sewage odour strongly indicates fecal contamination")
    if smell == 'chemical':
        causes.append("chemical smell suggests industrial or pesticide contamination")
    if appearance == 'brown':
        causes.append("brown/yellow colour indicates iron, rust, or manganese contamination")
    if appearance == 'greenish':
        causes.append("greenish colour indicates algal bloom — possible cyanotoxin risk")
    if deposits == 'oily':
        causes.append("oily film on surface indicates petroleum or chemical contamination")
    if deposits == 'reddish':
        causes.append("reddish-brown deposits suggest iron or rust from corroded pipes")
    if 'flooding' in events:
        causes.append("recent flooding significantly increases risk of sewage overflow")
    if source == 'borewell':
        causes.append("borewell water has no treatment — higher risk of bacterial contamination")
    if source == 'river':
        causes.append("river water carries high pathogen load without treatment")
...
def auto_alert_if_worsened(pincode, new_risk):
    try:
        previous = WaterData.objects.filter(
            pincode=pincode
        ...
        if has_worsened(old_risk, new_risk):
            users_resp = requests.get(
                f"http://127.0.0.1:5000/api/users-by-pincode/{pincode}",
                timeout=5
            )
            users = users_resp.json().get('users', [])
            for user_id in users:
                requests.post(
                    "http://127.0.0.1:5000/api/send-alert",
                    json={
                        "user_id": user_id,
                        "alert_type": "WATER_CONTAMINATION",
                        "severity": new_risk,
                        "details": {
                            "region": f"Pincode {pincode}",
...
# ── NEW HOME VIEW (replaces existing home) ────────────────
    ...
    if request.method == 'POST':
        ...
        try:
            resp = requests.post('http://127.0.0.1:5000/api/anomaly-check', json={
                'ph': ph or 7.2,
                'turbidity_ntu': turbidity or 5.0,
                'do_mg_l': do_val or temperature or 25.0,
                'bod_mg_l': bod or 5.0,
            }, timeout=5)
        ...
        if ph is not None:
            if ph < 6.5:
                analysis.append("Acidic water may corrode pipes and leach harmful metals.")
                risk_factors.append("acidic pH")
            elif ph > 8.5:
                analysis.append("Alkaline water may indicate contamination or mineral imbalance.")
                risk_factors.append("alkaline pH")
            else:
                safe_factors.append("balanced pH")

        if turbidity is not None:
            if turbidity > 5:
                analysis.append("High turbidity indicates suspended particles that can carry pathogens.")
                risk_factors.append("high turbidity")
            elif turbidity > 1:
                analysis.append("Moderate turbidity detected — filtration recommended.")
                risk_factors.append("moderate turbidity")
            else:
                safe_factors.append("clear water")

        if temperature is not None:
            if temperature > 30:
                analysis.append("Elevated temperature accelerates microbial growth.")
                risk_factors.append("high temperature")
            else:
                safe_factors.append("stable temperature")

        if tds is not None:
            if tds > 900:
                analysis.append("Very high dissolved solids — unsafe for drinking.")
                risk_factors.append("very high TDS")
            elif tds > 600:
                analysis.append("High TDS may affect taste and long-term health.")
                risk_factors.append("high TDS")
            else:
                safe_factors.append("acceptable TDS")

        if nitrate is not None:
            if nitrate > 50:
                analysis.append("High nitrate levels pose serious health risks, especially for infants.")
                risk_factors.append("high nitrate")
            elif nitrate > 30:
                analysis.append("Moderate nitrate levels — caution advised.")
                risk_factors.append("moderate nitrate")
            else:
                safe_factors.append("safe nitrate levels")

        # 🧠 SMART SUMMARY
        summary = f"Water quality is classified as {risk_level}. "

        if risk_factors:
            summary += "Key concerns include " + ", ".join(risk_factors[:3]) + ". "

        if safe_factors:
            summary += "Positive indicators such as " + ", ".join(safe_factors[:2]) + " are observed. "

        # ⚠️ ACTION
        if risk_level == "SEVERE":
            action = "Immediate intervention required. Avoid consumption completely and use an alternative safe source. RO/UV purification is strongly recommended."
        elif risk_level == "HIGH":
            action = "Water is unsafe for direct use. Boiling and filtration or RO treatment is required."
        elif risk_level == "MEDIUM":
            action = "Moderate contamination detected. Boiling or filtration is advised before use."
        else:
            action = "Water is within safe limits. Regular monitoring is recommended."
        ...
        WaterData.objects.create(
            check_type='detailed',
            ph=ph, turbidity=turbidity, temperature=temperature,
            risk_level=risk_level, quality=risk_level,
            pincode=pincode,
            summary=summary,
            disease='Detailed Analysis',
        )
...
def dashboard(request):
    ...
    for i in range(6, -1, -1):
        ...
        count = WaterData.objects.filter(
    ...
    pincode_data = list(
        WaterData.objects.values('pincode', 'risk_level')
