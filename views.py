...
# ── GROQ AI WATER ANALYSIS ────────────────────────────────

def get_ai_water_analysis(ph, turbidity, temperature, tds, nitrate, do_val, bod, risk_level):
    """
    Use Groq AI to generate intelligent water quality analysis.
    Falls back to rule-based summary if Groq is unavailable.
    """
    try:
        ...
        params = []
        if ph is not None:          params.append(f"pH: {ph}")
        if turbidity is not None:   params.append(f"Turbidity: {turbidity} NTU")
        if temperature is not None: params.append(f"Temperature: {temperature}°C")
        if tds is not None:         params.append(f"TDS: {tds} mg/L")
        if nitrate is not None:     params.append(f"Nitrate: {nitrate} mg/L")
        if do_val is not None:      params.append(f"Dissolved Oxygen: {do_val} mg/L")
        if bod is not None:         params.append(f"BOD: {bod} mg/L")
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "You are an expert water quality analyst. Analyze water parameters and provide "
                        "a clear, detailed health assessment. Use WHO guidelines. Be specific about risks. "
                        "Keep response under 150 words. Be direct and helpful."
                    )
                },
                {
                    "role": "user",
                    "content": (
                        f"Analyze these water quality parameters: {param_str}. "
                        f"Overall risk level is {risk_level}. "
                        f"Give a detailed health risk assessment and specific recommendations."
                    )
                }
            ],
            max_tokens=300,
        ...
        print(f"Groq water analysis error: {e}")
        return None


# ── LOCAL RISK CALCULATOR ─────────────────────────────────

def calculate_risk_level(ph, turbidity, temperature, tds, nitrate, do_val, bod):
    """Calculate risk level from parameters — no Flask needed."""
    score = 0

    if ph is not None:
        if ph < 5.0 or ph > 10.0:  score += 40
        elif ph < 6.0 or ph > 9.0: score += 25
        elif ph < 6.5 or ph > 8.5: score += 10

    if turbidity is not None:
        if turbidity > 50:   score += 40
        elif turbidity > 20: score += 25
        elif turbidity > 5:  score += 15

    if temperature is not None:
        if temperature > 45:   score += 30
        elif temperature > 35: score += 15
        elif temperature > 30: score += 8

    if tds is not None:
        if tds > 1200:   score += 35
        elif tds > 900:  score += 25
        elif tds > 600:  score += 12

    if nitrate is not None:
        if nitrate > 100:  score += 40
        elif nitrate > 50: score += 25
        elif nitrate > 30: score += 12

    if do_val is not None:
        if do_val < 2:   score += 30
        elif do_val < 4: score += 15
        elif do_val < 6: score += 8

    if bod is not None:
        if bod > 20:   score += 30
        elif bod > 10: score += 20
        elif bod > 5:  score += 10
    ...
    return risk_level, min(score, 100), score >= 45
...
def home(request):
    ...
    if request.method == 'POST':
        ...
        if score <= 30:       risk_level = 'Low Risk'
        elif score <= 70:     risk_level = 'Moderate Risk'
        elif score <= 110:    risk_level = 'High Risk'
        ...
        return render(request, 'home.html', {
            ...
            'risk_level': risk_level, 'answers': answers,
            'past_surveys': past_surveys,
        })

    return render(request, 'home.html', {'submitted': False, 'past_surveys': past_surveys})
...
def map_symptoms_to_disease(post):
    ...
    if 'watery_diarrhea' in symptoms:    scores['Cholera'] += 4
    if 'vomiting' in symptoms and 'watery_diarrhea' in symptoms: scores['Cholera'] += 2
    if 'bloody_diarrhea' in symptoms:    scores['Dysentery'] += 5
    if 'stomach_cramps' in symptoms:     scores['Dysentery'] += 2
    if 'diarrhea' in symptoms:           scores['Gastroenteritis'] += 2
    if 'vomiting' in symptoms:           scores['Gastroenteritis'] += 2
    if duration == 'less_24':            scores['Gastroenteritis'] += 3
    if fever_pattern == 'high_constant': scores['Typhoid'] += 4
    if duration in ['3_7_days', 'more_week']: scores['Typhoid'] += 2
    if 'fatigue' in symptoms:            scores['Typhoid'] += 1
    if 'jaundice' in symptoms:           scores['Hepatitis A'] += 6
    if 'fatigue' in symptoms:            scores['Hepatitis A'] += 2
    if 'bloating' in symptoms:           scores['Giardia'] += 4
    if duration == 'more_week':          scores['Giardia'] += 3
    if people in ['2_3_people', 'whole_household']:
        scores['Cholera'] += 2; scores['Gastroenteritis'] += 2
    if after_water == 'yes':
        for k in scores: scores[k] *= 1.2
    if water_source in ['surface', 'borewell']:
        scores['Cholera'] += 1; scores['Giardia'] += 1; scores['Dysentery'] += 1
    if treatment == 'never':
        for k in scores: scores[k] *= 1.2
    if sanitation == 'poor':
        scores['Dysentery'] += 2; scores['Typhoid'] += 1
    ...
    if confidence > 70 or severity_boost: severity = 'HIGH'
    ...
    return {
        'disease': top_disease, 'confidence': confidence, 'severity': severity,
        'probabilities': {k: round(v * 100, 1) for k, v in probabilities.items()},
        'info': disease_info.get(top_disease, {}), 'severity_boost': severity_boost,
    }
...
def chatbot(request):
    ...
    if request.method == 'POST':
        ...
        if message:
            ...
            try:
                bot = get_chatbot()
                # ✅ Pass chat history for context-aware AI responses
                result = bot.chat(message, chat_history=request.session['chat_history'])
...
def history(request):
    ...
    return render(request, 'history.html', {
        'records': records, 'low_count': low_count,
        'moderate_count': moderate_count, 'high_count': high_count,
...
def get_quick_cause(post):
    smell = post.get('smell', 'none'); appearance = post.get('appearance', 'clear')
    deposits = post.get('deposits', 'none'); source = post.get('source', 'municipal')
    events = post.getlist('events'); causes = []
    if smell == 'rotten_egg':    causes.append("rotten egg smell suggests hydrogen sulphide")
    if smell == 'sewage':        causes.append("sewage odour strongly indicates fecal contamination")
    if smell == 'chemical':      causes.append("chemical smell suggests industrial contamination")
    if appearance == 'brown':    causes.append("brown colour indicates iron or rust contamination")
    if appearance == 'greenish': causes.append("greenish colour indicates algal bloom")
    if deposits == 'oily':       causes.append("oily film indicates petroleum contamination")
    if deposits == 'reddish':    causes.append("reddish deposits suggest iron from corroded pipes")
    if 'flooding' in events:     causes.append("flooding increases risk of sewage overflow")
    if source == 'borewell':     causes.append("borewell water has higher bacterial contamination risk")
    if source == 'river':        causes.append("river water carries high pathogen load")
    if not causes:
        return "No specific contamination indicators detected."
        ...
        if has_worsened(old_risk, new_risk):
            ...
            for user_id in users:
                requests.post("http://127.0.0.1:5000/api/send-alert", json={
                    "user_id": user_id, "alert_type": "WATER_CONTAMINATION", "severity": new_risk,
...
# ── DETAILED ANALYSIS VIEW (AI-powered) ──────────────────
...
def detailed_analysis(request):
    if request.method == 'POST':
        ...
        # ✅ Step 1: Calculate risk level locally
        risk_level, anomaly_score, is_anomaly = calculate_risk_level(
            ph, turbidity, temperature, tds, nitrate, do_val, bod
        )

        # ✅ Step 2: Get AI-generated analysis from Groq
        ai_summary = get_ai_water_analysis(
            ph, turbidity, temperature, tds, nitrate, do_val, bod, risk_level
        )
        ...
        if do_val is not None:
            if do_val < 4:   analysis.append("Very low dissolved oxygen indicates heavy organic pollution."); risk_factors.append("low dissolved oxygen")
            elif do_val < 6: analysis.append("Low dissolved oxygen detected."); risk_factors.append("low DO")
            else:            safe_factors.append("adequate dissolved oxygen")

        if bod is not None:
            if bod > 10:  analysis.append("High BOD indicates significant organic pollution."); risk_factors.append("high BOD")
            elif bod > 5: analysis.append("Moderate BOD detected."); risk_factors.append("moderate BOD")
            else:         safe_factors.append("low organic pollution")

        # ✅ Use AI summary if available, else build rule-based summary
        if ai_summary:
            summary = ai_summary
        else:
            summary = f"Water quality is classified as {risk_level}. "
            if risk_factors: summary += "Key concerns include " + ", ".join(risk_factors[:3]) + ". "
            if safe_factors: summary += "Positive indicators: " + ", ".join(safe_factors[:2]) + "."

        if risk_level == "SEVERE":   action = "Immediate intervention required. Avoid consumption completely. RO/UV purification strongly recommended."
        elif risk_level == "HIGH":   action = "Water is unsafe for direct use. Boiling and filtration or RO treatment required."
        elif risk_level == "MEDIUM": action = "Moderate contamination detected. Boiling or filtration advised before use."
        else:                        action = "Water is within safe limits. Regular monitoring recommended."
        ...
        return render(request, 'result.html', {
            ...
            'ai_powered': ai_summary is not None,  # ✅ Tell template if AI was used
        })
...
def dashboard(request):
    ...
    return render(request, 'dashboard.html', {
        'total_checks': total_checks, 'today_checks': today_checks,
        'pincode_count': pincode_count, 'alerts_sent': alerts_sent,
        'risk_counts': json.dumps(risk_counts), 'risk_counts_raw': risk_counts,
        'trend_labels': json.dumps(trend_labels), 'trend_data': json.dumps(trend_data),
        'pincode_data': pincode_data, 'pincode_data_json': json.dumps(pincode_data),
        'quick_count': quick_count, 'detailed_count': detailed_count,
        'recent': recent,
    })
